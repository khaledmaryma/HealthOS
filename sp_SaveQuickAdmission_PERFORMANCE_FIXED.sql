-- =====================================================
-- Stored Procedure: sp_SaveQuickAdmission (PERFORMANCE FIXED VERSION)
...
BEGIN
    ...
    DECLARE @department NVARCHAR(50); -- Department is a string, not INT
    ...
    BEGIN TRY
        -- Parse save options - Extract from nested saveOptions if @saveOptions contains full JSON
        DECLARE @optionsJson NVARCHAR(MAX);
        
        IF @saveOptions IS NOT NULL AND @saveOptions != '' AND @saveOptions != 'null'
        BEGIN
            -- Check if @saveOptions contains nested saveOptions (full JSON structure)
            SET @optionsJson = JSON_QUERY(@saveOptions, '$.saveOptions');
            
            -- If not found as nested, check if it's already the options JSON (has saveMedicalFile at root)
            IF @optionsJson IS NULL OR @optionsJson = 'null' OR @optionsJson = ''
            BEGIN
                IF JSON_VALUE(@saveOptions, '$.saveMedicalFile') IS NOT NULL
                BEGIN
                    SET @optionsJson = @saveOptions; -- It's already the options JSON
                END
                ELSE IF @saveData IS NOT NULL
                BEGIN
                    SET @optionsJson = JSON_QUERY(@saveData, '$.saveOptions'); -- Try from saveData
                END
            END
        END
        ELSE
        ...
        BEGIN
            ...
            SET @department = JSON_VALUE(@saveData, '$.admission.Department'); -- Department is NVARCHAR, not INT
            ...
            -- Generate admission number using stored procedure (your change)
        ...
        -- =====================================================
        -- 3. SAVE INVOICE (if requested) - PERFORMANCE FIXED
        -- =====================================================
        IF @saveInvoice = 1 AND @admissionId IS NOT NULL AND @saveData IS NOT NULL
        BEGIN
            -- Get invoice JSON once (PERFORMANCE FIX: Extract JSON once, don't scan multiple times)
            DECLARE @invoiceJson NVARCHAR(MAX) = JSON_QUERY(@saveData, '$.invoice');
            
            -- FAST CHECK: Just check if JSON exists and is not empty (no OPENJSON scan!)
            IF @invoiceJson IS NOT NULL AND @invoiceJson != 'null' AND @invoiceJson != '' AND @invoiceJson != '[]'
            BEGIN
                -- Calculate totals using OPENJSON (single call)
                ...
                -- PERFORMANCE FIX: Single OPENJSON call to calculate totals
                SELECT
                ...
                -- PERFORMANCE FIX: Single OPENJSON call for invoice details insertion (reuse @invoiceJson)
        ...
        -- =====================================================
        IF @transactionCount = 0
            COMMIT TRANSACTION; -- FIXED: Was ROLLBACK, now COMMIT
