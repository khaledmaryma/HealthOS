...
-- First, ensure AdmissionCounter table exists
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'AdmissionCounter')
BEGIN
    PRINT 'Creating AdmissionCounter table...';
    ...
    PRINT 'AdmissionCounter table created successfully';
END
GO
...
BEGIN
    ...
    BEGIN
        ...
        SET @MonthPart = RIGHT('0' + CAST(MONTH(@CheckInDate) AS NVARCHAR(2)), 2);
        SET @YearPart = RIGHT(CAST(YEAR(@CheckInDate) AS NVARCHAR(4)), 2);
